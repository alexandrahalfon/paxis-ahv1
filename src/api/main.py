...
async def shutdown():
    """Close database connection pools gracefully on shutdown."""
    try:
        from src.api.services.account_db import get_account_db
        await get_account_db().close()
        print("[Shutdown] Accounts DB pool closed")
    except Exception as e:
        print(f"[Shutdown] Accounts DB pool close failed: {e}")

    try:
        from src.api.services.patient_db import get_patient_db
        await get_patient_db().close()
        print("[Shutdown] Patients DB pool closed")
    except Exception as e:
        print(f"[Shutdown] Patients DB pool close failed: {e}")

    try:
        from src.api.services.postgres_study_details_service import get_postgres_study_details_service
        await get_postgres_study_details_service().close()
        print("[Shutdown] Study Details DB pool closed")
    except Exception as e:
        print(f"[Shutdown] Study Details DB pool close failed: {e}")
