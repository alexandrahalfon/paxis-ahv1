...
async def get_valid_doc_ids_from_preferences(preferences: Dict[str, Any]) -> Optional[Set[str]]:
    ...
    try:
        ...
        # Uses the same host/port/user/password as the main study DB since
        # study-profiles lives on the same PostgreSQL server.
        conn = await asyncpg.connect(
            host=settings.postgres_host,
            port=settings.postgres_port,
            user=settings.postgres_user,
            password=settings.postgres_password,
            database=settings.study_profiles_database,
            timeout=5,
            command_timeout=10,
        )
